...
qpal = colorNumeric(palette = "Reds", counties_sf_join_converted$PovertyPerc)
...
countymap_poverty <- leaflet(counties_percent) %>%
  ...
  addLegend("bottomright", values=~PovertyPerc,pal=qpal,title="Percent in Poverty", labFormat = labelFormat(prefix = "%"), opacity = 1)

countymap_poverty

## PERCENT MINORITY MAP BY COUNTY
qpal = colorNumeric( palette = "Reds", domain = counties_sf_join_converted$MinorityPerc)

countymap_minority <- leaflet(counties_percent) %>%
  addPolygons(stroke = TRUE,opacity = 1,fillOpacity = 0.5, smoothFactor = 0.5,
              color="black",fillColor = ~qpal(MinorityPerc),weight = 1) %>%
  addLegend("bottomright", values=~MinorityPerc,pal=qpal,title="Percent Minority", labFormat = labelFormat(prefix = "%"), opacity = 1)
...
qpal2 <- colorNumeric(palette = "RdYlBu", counties_percent$MinorityPerc)

hog_farm_map1 <- leaflet() %>% 
  addCircleMarkers(data = hog_farms.sf2, radius = 2, color = "blue", stroke = FALSE, fillOpacity = 0.9) %>%
  addLegend(data = hog_farms.sf2, values=~Type,pal=qpal,title="Farm Type") %>%
  ...
  addLegend("bottomright", data =counties_percent, values=~qpal2,pal=qpal,title="Percent Minority", labFormat = labelFormat(prefix = "%"), opacity = 1)
...
solar_map1 <- leaflet() %>% 
  addCircleMarkers(data = hog_farms.sf2, radius = 2, color = "purple", stroke = FALSE, fillOpacity = 0.9) %>%
  addLegend(data = hog_farms.sf2, values=~Type,pal=qpal,title="Farm Type") %>%
  addCircleMarkers(data = solar_plants_NC.sf, radius = 2, color = "green", stroke = FALSE, fillOpacity = 0.9) %>%
  addLegend(data = solar_plants_NC.sf, values=~"Plant Name",pal=qpal,title="Solar Farm Name") %>%
  addPolygons(data =counties_percent, stroke = TRUE,opacity = 1,fillOpacity = 0.5, smoothFactor = 0.5,
              color="black",fillColor = ~qpal(MinorityPerc),weight = 1) %>%
    addProviderTiles("Esri.WorldGrayCanvas",
        options = providerTileOptions(minZoom=3, maxZoom=8))  %>%
    addLabelOnlyMarkers(data = hog_farms.sf2, label = ~Facility) %>%
  addLegend(data =counties_percent, values=~qpal2,pal=qpal,title="Percent Minority")
